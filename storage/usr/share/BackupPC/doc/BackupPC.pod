...
This documentation describes BackupPC version 3.1.0,
released on 25 Nov 2007.
...
Therefore BackupPC's data store (/var/lib/BackupPC/) must point to a single
...
symbolic link at the top-level directory (/var/lib/BackupPC/) to point the
...
available, eg: BackupPC-3.1.0pl2.diff, you should apply
...
rather than below the data directory, /var/lib/BackupPC//conf,
and the log files will be stored in /var/log/BackupPC
rather than below the data directory, /var/lib/BackupPC//log.
...
On this installation, this is /usr/share/BackupPC/sbin/.
...
located in the following locations:
    ...
    /var/log/BackupPC/BackupPC      log files, pid, status
...
/etc/BackupPC//config.pl, and make sure all the default settings
...
The file /etc/BackupPC//hosts contains the list of clients to backup.
...
/etc/BackupPC//config.pl.  If you put the password
...
configuration file (/etc/BackupPC//pc/$host.pl or
/var/lib/BackupPC//pc/$host/config.pl in non-FHS versions of BackupPC).
...
as user backuppc.  The -d option tells BackupPC to run as a daemon
(ie: it does an additional fork).

Any immediate errors will be printed to stderr and BackupPC will quit.
Otherwise, look in /var/log/BackupPC/LOG and verify that BackupPC reports
...
should be run as the BackupPC user (backuppc), so you
should
...
in /etc/BackupPC//hosts as part of a big cryptic output line.
...
to be run as the BackupPC user (backuppc).  This is
...
in /usr/share/BackupPC/sbin//BackupPC_Admin.  BackupPC_Admin should have been installed
as setuid to the BackupPC user (backuppc), in addition to user
and group execute permission.

You should be very careful about permissions on BackupPC_Admin and
the directory /usr/share/BackupPC/sbin/: it is important that normal users cannot
...
The permissions should look like this:
    ...
    ls -l /usr/share/BackupPC/sbin//BackupPC_Admin
    -swxr-x---    1 backuppc   web      82406 Jun 17 22:58 /usr/share/BackupPC/sbin//BackupPC_Admin
...
To use mod_perl you need to run Apache as user backuppc.
...
To copy a complete store (ie: /var/lib/BackupPC/) using BackupPC_tarPCCopy
you should:
...
/usr/share/BackupPC/BackupPC_archiveHost.  This script can be copied
...
/etc/BackupPC//config.pl. It then runs and manages all the backup
...
the host name.  The file /var/lib/BackupPC//pc/$host/backups is read to decide
...
extracted to /var/lib/BackupPC//pc/$host/new.  The XferMethod output is put
into /var/lib/BackupPC//pc/$host/XferLOG.
...
disk and the file name is saved in /var/lib/BackupPC//pc/$host/NewFileList for
...
are moved to /var/lib/BackupPC//trash for later removal by BackupPC_trashClean.
...
=item /usr/share/BackupPC

Perl scripts comprising BackupPC reside in /usr/share/BackupPC/bin,
libraries are in /usr/share/BackupPC/lib and documentation
is in /usr/share/BackupPC/doc.

=item /usr/share/BackupPC/sbin/

The CGI script BackupPC_Admin resides in this cgi binary directory.

=item /etc/BackupPC/

All the configuration information resides below /etc/BackupPC/.
This directory contains:

The directory /etc/BackupPC/ contains:
...
The directory /etc/BackupPC//pc contains per-client configuration files
that override settings in the main configuration file.  Each file
is named /etc/BackupPC//pc/HOST.pl, where HOST is the host name.
...
=item /var/log/BackupPC

The directory /var/log/BackupPC (/var/lib/BackupPC//log on pre-FHS versions
...
/var/lib/BackupPC//trash for later removal by BackupPC_trashClean.

=item /var/lib/BackupPC//pool

All uncompressed files from PC backups are stored below /var/lib/BackupPC//pool.
...
the file is stored in /var/lib/BackupPC//pool/1/2/3/123456789abcdef0.
...
(one for the pool file and one for the backup file below /var/lib/BackupPC//pc).
...
=item /var/lib/BackupPC//cpool

All compressed files from PC backups are stored below /var/lib/BackupPC//cpool.
Its layout is the same as /var/lib/BackupPC//pool, and the hashing function
is the same (and, importantly, based on the uncompressed file, not
the compressed file).

=item /var/lib/BackupPC//pc/$host

For each PC $host, all the backups for that PC are stored below
the directory /var/lib/BackupPC//pc/$host.  This directory contains the
following files:
...
stored in /etc/BackupPC//pc/HOST.pl.
...
stored in /var/lib/BackupPC//pc/$host/5).
...
BackupPC_zcat can be found in /usr/share/BackupPC/bin.  For each
...
The BackupPC configuration file resides in /etc/BackupPC//config.pl.
Optional per-PC configuration files reside in /etc/BackupPC//pc/$host.pl
(or /var/lib/BackupPC//pc/$host/config.pl in non-FHS versions of BackupPC).
...
The optional per-PC configuration file (/etc/BackupPC//pc/$host.pl or
/var/lib/BackupPC//pc/$host/config.pl in non-FHS versions of BackupPC)
...
=item $Conf{BackupPCdPath} = '';

Path to backuppcd executable on the server

=item $Conf{BackupPCdCmd} = '$bpcdPath $host $shareName $poolDir XXXX $poolCompress $topDir/pc/$client/new';
...
=item $Conf{BackupPCdRestoreCmd} = '$bpcdPath TODO';
