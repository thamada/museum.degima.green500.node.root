        ...
        if avc.matches_source_types(['ftpd_t']) and \
           ...
           avc.has_tclass_in(['file', 'dir']):
            ...
            if avc.all_accesses_are_in(avc.r_file_perms + avc.r_dir_perms):
                # MATCH
                avc.set_template_substitutions(BOOLEAN="allow_ftpd_use_cifs", ACCESS="reading")
                ...
                # MATCH
                avc.set_template_substitutions(BOOLEAN="allow_ftpd_use_cifs", ACCESS="reading and writing",
                                               WRITE_BOOLEAN="allow_ftpd_anon_write")
                return self.report(avc, _("FTP"),
                                   self.summary, self.problem_description,
                                   self.rw_fix_description, self.rw_fix_cmd)
            else:
                return None
        else:
            return None
