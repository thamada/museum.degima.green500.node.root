...
class plugin(Plugin):
    ...
    def analyze(self, avc):
        if avc.matches_source_types(['httpd_t'])           and \
           avc.matches_target_types(['.*script_exec_t$'])  and \
           avc.all_accesses_are_in(['read', 'getattr', 'search', 'execute', 'transition', 'execute_no_trans']):
            # MATCH
            avc.set_template_substitutions(BOOLEAN="httpd_enable_cgi")
