...
class plugin(Plugin):
    ...
    def analyze(self, avc):
        if avc.matches_target_types(['nfs_t']) and \
           avc.has_tclass_in(['file', 'dir']):
            if avc.all_accesses_are_in(avc.r_file_perms + avc.r_dir_perms):
                # MATCH
                avc.set_template_substitutions(BOOLEAN="use_nfs_home_dirs", ACCESS="reading")
            ...
            if avc.all_accesses_are_in(avc.create_file_perms + avc.rw_dir_perms):
                # MATCH
                avc.set_template_substitutions(BOOLEAN="use_nfs_home_dirs", ACCESS="reading and writing")
                return self.report(avc, _("File System"),
                                   self.summary, self.problem_description,
                                   self.fix_description, self.fix_cmd)
        return None
