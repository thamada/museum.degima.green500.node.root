...
#
# Modified to add recent files and gnomevfs
# Gustavo Carneiro <gjc@gnome.org>, 2004
#
...
class Window(gtk.Window):
    def __init__(self):
        ...
        self.set_title('GtkUIManager/RecentViewGtk test app')
        ...
        ## Now the important stuff: add recent files
	model = egg.recent.RecentModel(egg.recent.RECENT_MODEL_SORT_MRU)
        open = self.ui.get_widget("/ui/Menubar/FileMenu/Open")
        self.recent = egg.recent.RecentViewGtk(open.parent, open)
	self.recent.set_model(model)

        self.recent.connect("activate", self.recent_activate_cb)

    def recent_activate_cb(self, rview, item):
        self.load_uri(item.get_uri())
    ...
    def file_open_cb(self, action):
        ...
        if dialog.run() == gtk.RESPONSE_OK:
            self.load_uri(dialog.get_uri())

        dialog.destroy()

    def load_uri(self, uri):
        self.buffer.set_text(gnomevfs.read_entire_file(uri))
        self.recent.get_model().add(uri)
