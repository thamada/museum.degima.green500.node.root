...
# -*- coding: utf-8 -*-
#
# Copyright (C) 2002 - 2005, 2008, 2009 Red Hat, Inc.
...
class AddUserWin(object):

    def __init__(self, samba_user_win, samba_backend, xml):
        ...
        self.user_add_label = xml.get_widget ("user_add_label")
        self.user_option_menu = xml.get_widget("user_option_menu")
        self.user_edit_label = xml.get_widget ("user_edit_label")
        self.user_name_label = xml.get_widget ("user_name_label")
    ...
    def showEditWindow(self, unix_name, windows_name):
        ...
        self.add_user_win.show_all()
        self.user_add_label.hide ()
        self.user_option_menu.hide ()
    ...
    def on_edit_user_ok_button_clicked(self, *args):
        ...
        if self.orig_windows_name != windows_name:
            self.samba_backend.changeWindowsUserName(self.orig_unix_name, windows_name)

        if password != "********":
            #The passwords match, so let's update the user's password
            result = self.samba_backend.changePassword(self.orig_unix_name, password)
    ...
    def on_add_user_ok_button_clicked(self, *args):
        ...
        if windows_name.strip() == "":
            ...
            self.samba_backend.addUser(unix_name, windows_name, password)
